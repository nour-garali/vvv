  ...
  const handleSaveAvatar = async (values) => {
    if (editor && avatar) {
      ...
      canvas.toBlob(async (blob) => {
        ...
        try {
          const response = await fetch('http://127.0.0.1:5000/signup_conseilleur', {
            method: 'POST',
            body: formData,
          });

          if (response.ok) {
            console.log('conseillerLocale created successfully');
          } else {
            console.error('Failed to create President:', response.statusText);
          }
        } catch (error) {
...
export default SignupConseilleur;
